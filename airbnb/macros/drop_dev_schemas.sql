{% macro drop_dev_schemas() %}
	...
	{# Use the resolved target schema as the prefix (e.g. DBT_<env_name> on the dev)#}
	...
	{{ log(" * drop_dev_schemas : Looking for schema with prefix: " ~ prefix, information_schema)}}
	...
	{% if execute %}
		...
		{% if schemas | length == 0 %}
			{{ log(" * drop_dev_schemas: No schemas found matching prefix: " ~ prefix, information_schema)}}
		{% else %}
			{{ log(" * drop_dev_schemas: Found " ~ schemas | length ~ " schema(s) to drop") }}
